...
namespace impactgen {
    ...
    ExtremeTemperaturesPerSubregion::ExtremeTemperaturesPerSubregion(const settings::SettingsNode &impact_node,
    ...
    void ExtremeTemperaturesPerSubregion::join(Output &output, const TemplateFunction &template_func) {
        ...
        struct RegionParameters {
            std::vector<SectorParameter> quadratic_factor_cold;  // vector of a*(cold_temperature_deviation)^2
            std::vector<SectorParameter> quadratic_factor_heat; // vector of b*(heat_temperature_deviation)^2
            std::vector<SectorParameter> cold_deviation_mean; // temperature below long run mean below which cold stress starts
            std::vector<SectorParameter> heat_deviation_mean; // temperature above long run mean above which heat stress starts
            std::vector<SectorParameter> baseline; // (positive) productivity shift around the long run mean
        };
        ...
        for (const auto &parameters_current_region: parameters.as_map()) {
            ...
            for (const auto &node: parameters_current_region.second["quadratic_factor_cold"].as_map()) {
                parameters_struct.quadratic_factor_cold.push_back(
                        {all_sectors.at(node.first), node.second.as<ForcingType>()});
            }
            for (const auto &node: parameters_current_region.second["quadratic_factor_heat"].as_map()) {
                parameters_struct.quadratic_factor_heat.push_back(
                        {all_sectors.at(node.first), node.second.as<ForcingType>()});
            }
            for (const auto &node: parameters_current_region.second["cold_deviation_mean"].as_map()) {
                parameters_struct.cold_deviation_mean.push_back(
                        {all_sectors.at(node.first), node.second.as<ForcingType>()});
            }
            for (const auto &node: parameters_current_region.second["heat_deviation_mean"].as_map()) {
                parameters_struct.heat_deviation_mean.push_back(
                        {all_sectors.at(node.first), node.second.as<ForcingType>()});
            }
            for (const auto &node: parameters_current_region.second["baseline"].as_map()) {
                parameters_struct.baseline.push_back(
                        {all_sectors.at(node.first), node.second.as<ForcingType>()});
            }
                        ...
                        for (const auto &quadratic_factor_cold: parameters_current_region.quadratic_factor_cold) { //iterate over sectors using parameters
                            ...
                            if (forcing_v < parameters_current_region.cold_deviation_mean[i_sector].value) {
                                cold_forcing = parameters_current_region.quadratic_factor_cold[i_sector].value *
                                               pow(forcing_v -
                                                   parameters_current_region.cold_deviation_mean[i_sector].value, 2);
                            }

                            if (forcing_v > parameters_current_region.heat_deviation_mean[i_sector].value) {
                                heat_forcing = parameters_current_region.quadratic_factor_heat[i_sector].value *
                                               pow(forcing_v -
