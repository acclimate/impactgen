...
#include "impacts/HeatedProductivity.h"
#include <algorithm>
#include <cmath>
#include <iostream>
#include <stdexcept>
...
HeatedProductivity::HeatedProductivity(const settings::SettingsNode& impact_node, AgentForcing base_forcing_p)
    ...
    for (const auto category_node : impact_node["worker_categories"].as_sequence()) {
        std::vector<int> sectors;
        for (const auto sector_node : category_node["sectors"].as_sequence()) {
            sectors.push_back(all_sectors.at(sector_node.as<std::string>()));
        }
